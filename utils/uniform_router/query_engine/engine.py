...
from .operators import icontains
from .strategies import BaseStrategy
from .types import LookupExpr, ModelAttribute

if TYPE_CHECKING:
    from .filter_set import BaseFilterSet  # pragma: no cover


class BaseEngine:
    """A Base class for all filters
    Attributes
        field_name: Name of Engine in EngineSet. Set by EngineSet after creation.
    """
    ...
    def filter(self, query: Select, value: Any, values: Dict[str, Any]) -> Select:
        """Implementation of query build for this Engine"""
        ...  # pragma: no cover


class Engine(BaseEngine):
    """Engine results by field, value and lookup_expr"""
...
class OrderingFilter(BaseEngine):
...
class LimitOffsetFilter(BaseEngine):
    """Engine for managing limit and offset"""
    ...
    def filter(
        ...
        Example::
            LimitOffsetEngine(select(Item), value=(100, 0))
        """
...
class MethodEngine(BaseEngine):
    """This helper is used to override Engine.filter() when a 'method' argument
    is passed. It proxies the call to the actual method on the filter's parent filterset.
    """

    def __init__(self, method: str) -> None:
        """
        :param method: Method name in parent EngineSet
        """
        super().__init__()
        self.method = method
    ...
    def init_filter_method(self) -> None:
        from .filter_set import BaseFilterSet
...
class SearchEngine(BaseEngine):
    """Engine for searching by a given search string"""
