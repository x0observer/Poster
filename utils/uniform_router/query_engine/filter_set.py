...
class FilterSetMetaclass(abc.ABCMeta):
    ...
    @classmethod
    def get_declared_filters(mcs, bases: tuple, attrs: Dict[str, Any]) -> Dict[str, BaseEngine]:
        filters: Dict[str, BaseEngine] = OrderedDict()
        ...
        for filter_name, filter_ in list(attrs.items()):
            if not isinstance(filter_, BaseEngine):
                continue
...
class BaseFilterSet(Generic[Model], metaclass=FilterSetMetaclass):
    declared_filters: Dict[str, BaseEngine]
    ...
    @classmethod
    def get_filters(cls) -> Dict[str, BaseEngine]:
